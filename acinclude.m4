...
#include <signal.h>],
    [int sa = SIGALRM;
     void (*a) = alarm;],
   [AC_DEFINE([USE_ALARM], [1], [Use alarm])])
 fi])
#
# ----------------------------------------------------------------
...
#include <sys/socket.h>
	...
	[AC_MSG_RESULT(no, but __ss_family exists)
	 AC_DEFINE([HAVE_OLD_SS_FAMILY], [1], [Have old ss_family])],
	[AC_MSG_RESULT(no)
...
[have_sigsetjmp="yes"; AC_DEFINE([HAVE_SIGSETJMP], [1], [Have sigsetjmp])],
[have_sigsetjmp="no"])
...
[AC_TYPE_SIGNAL
 ...
 if test x"$ac_cv_type_signal" = xvoid; then
  AC_DEFINE([SIGNAL_RETURN], [return], [Signal return type])
 else
  AC_DEFINE([SIGNAL_RETURN], [return 0], [Signal return type])
 fi])
